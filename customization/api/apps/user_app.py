...
### Add Azure AD OAuth
@manager.route('/azure_callback', methods=['GET'])
def azure_callback():
    import requests

    res = requests.post(settings.AZURE_OAUTH.get("token_url"),
                        data={
    ...
    user_info = user_info_from_azure(res["access_token"], res["id_token"])
    email_address = user_info["email"]
    users = UserService.query(email=email_address)
    user_id = get_uuid()
    if not users:
...
def user_info_from_azure(access_token, id_token):
    import jwt
    res = jwt.decode(id_token, options={"verify_signature": False}) | jwt.decode(access_token, options={"verify_signature": False})
    return res